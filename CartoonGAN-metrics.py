...
parser.add_argument('--vgg_model', required=False, default='VGG19/vgg19.pth', help='pre-trained VGG19 model path')
...
parser.add_argument('--disc_every', type=int, default=1)
parser.add_argument('--gen_every', type=int, default=1)
...
if not os.path.isdir(os.path.join(args.name + '_results', 'Transfer-New')):
    os.makedirs(os.path.join(args.name + '_results', 'Transfer-New'))
...
if args.latest_generator_model == '':
    ...
    if isinstance(G, nn.DataParallel):
        torch.save(G.module.state_dict(), os.path.join(args.name + '_results', 'generator_latest.pkl'))
    else:
...
train_hist['Gen_loss'] = []
train_hist['Con_loss'] = []
train_hist['Disc_real_acc'] = []
train_hist['Disc_fake_acc'] = []
train_hist['Disc_real_edge_acc'] = []
train_hist['per_epoch_time'] = []
train_hist['total_time'] = []

epoch_hist = {}
epoch_hist['Disc_loss'] = []
epoch_hist['Gen_loss'] = []
epoch_hist['Con_loss'] = []
epoch_hist['Disc_real_acc'] = []
epoch_hist['Disc_fake_acc'] = []
epoch_hist['Disc_real_edge_acc'] = []
...
#     D_scheduler.step()
    ...
    for (x, _), (y, _) in zip(train_loader_src, train_loader_tgt):
        ...
        if epoch % args.disc_every == 0:
            ...
            d_real_blur_acc.append(torch.mean(1.0-D_edge).item())

            train_hist['Disc_real_acc'].append(torch.mean(D_real).item())
            train_hist['Disc_fake_acc'].append(torch.mean(1.0-D_fake).item())
            train_hist['Disc_real_edge_acc'].append(torch.mean(1.0-D_edge).item())
    ...
    print("Disc real acc: %.3f, Disc fake acc: %.3f, Disc real blur acc: %.3f" % 
              (np.mean(d_real_acc), np.mean(d_fake_acc), np.mean(d_real_blur_acc)))
    
    epoch_hist['Disc_loss'].append(np.mean(Disc_losses))
    epoch_hist['Gen_loss'].append(np.mean(Gen_losses))
    epoch_hist['Con_loss'].append(np.mean(Con_losses))
    epoch_hist['Disc_real_acc'].append(np.mean(d_real_acc))
    epoch_hist['Disc_fake_acc'].append(np.mean(d_fake_acc))
    epoch_hist['Disc_real_edge_acc'].append(np.mean(d_real_blur_acc))
                                   
    hist_df = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in train_hist.items() ]))
    hist_df.to_csv(os.path.join(args.name + '_results', "hist.csv"), encoding='utf-8', index=False) 
    
    met_df = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in epoch_hist.items() ]))
    met_df.to_csv(os.path.join(args.name + '_results', "metrics.csv"), encoding='utf-8', index=False)
    ...
    if epoch % 2 == 1 or epoch == args.train_epoch - 1:
        with torch.no_grad():
            ...
            for n, (x, _) in enumerate(train_loader_src):
                ...
                G_recon = G(x)
                result = torch.cat((x[0], G_recon[0]), 2)
                path = os.path.join(args.name + '_results', 'Transfer-New', str(epoch+1) + '_epoch_' + args.name + '_train_' + str(n + 1) + '.png')
                plt.imsave(path, (result.cpu().numpy().transpose(1, 2, 0) + 1) / 2)
                if n == 4:
                    break
            ...
            for n, (x, _) in enumerate(test_loader_src):
                ...
                G_recon = G(x)
                result = torch.cat((x[0], G_recon[0]), 2)
                path = os.path.join(args.name + '_results', 'Transfer-New', str(epoch+1) + '_epoch_' + args.name + '_test_' + str(n + 1) + '.png')
                plt.imsave(path, (result.cpu().numpy().transpose(1, 2, 0) + 1) / 2)
                if n == 4:
                    break
            if isinstance(G, nn.DataParallel):
            ...
            if isinstance(D, nn.DataParallel):
                torch.save(D.module.state_dict(), os.path.join(args.name + '_results', 'discriminator_latest.pkl'))
            else:
...
if isinstance(G, nn.DataParallel):
    torch.save(G.module.state_dict(), os.path.join(args.name + '_results',  'generator_param.pkl'))
else:
    torch.save(G.state_dict(), os.path.join(args.name + '_results',  'generator_param.pkl'))
if isinstance(D, nn.DataParallel):
    torch.save(D.module.state_dict(), os.path.join(args.name + '_results',  'discriminator_param.pkl'))
else:
